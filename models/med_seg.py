...
from .backbone.UMamba import UMambaEncoder
from .backbone.feature_dfuse import FeatureEnhancement
from .backbone.Vim import Vimamba
from .backbone.Vit_model import Vit
from .backbone.ViLSTM import ViL
# DEBUG
...
class FewShotSeg(nn.Module):
    ...
    def __init__(self, in_channels=3, pretrained_path=None, cfg=None):
        ...
        self.get_cls()
        self.FeatureEnhancement = FeatureEnhancement()
    ...
    def get_encoder(self, in_channels):
        ...
        if self.config['which_model'] == 'dlfcn_res101':
            ...
            self.resnet_encoder = TVDeeplabRes101Encoder(use_coco_init)
            self.ViL=ViL()
            #self.Vit =Vit()
            #self.UMambaEncoder = UMambaEncoder()
            #self.Vimamba =Vimamba()
    ...
    def get_cls(self):
        ...
        """
        proto_hw = self.config["proto_grid_size"]  # proto_grid_size=8
    ...
    def forward(self, supp_imgs, fore_mask, back_mask, qry_imgs, isval, val_wsize, show_viz = False):
        ...
        # print('sup_bsize:', sup_bsize, 'qry_bsize:', qry_bsize)
        assert sup_bsize == qry_bsize == 1

        imgs_concat = torch.cat([torch.cat(way, dim=0) for way in supp_imgs]
                                + [torch.cat(qry_imgs, dim=0),], dim=0)

        img_fts_resunet = self.resnet_encoder(imgs_concat, low_level = False)
        img_fts_vil = self.ViL(imgs_concat)
        #img_fts_vit = self.Vit(imgs_concat)
        #img_fts_umamba =self.UMambaEncoder(imgs_concat) # (2,256,32,32)
        # x_resized = F.interpolate(imgs_concat, size=(224, 224), mode='bilinear', align_corners=False)
#        img_fts_Vimamba = self.Vimamba(imgs_concat)
        # y = F.interpolate(img_fts_Vimamba, size=(32, 32), mode='bilinear', align_corners=False)
        #img_fts=self.FeatureEnhancement(img_fts_resunet, img_fts_vil)
        img_fts = img_fts_vil+img_fts_resunet
        # print("img_fts:", img_fts)
        # print("img_fts:", img_fts_vil)
        # print("img_fts_resunet:",img_fts_resunet)
