...
class SuperpixelDataset(BaseDataset):
    def __init__(self, which_dataset, base_dir, idx_split, mode, transforms, scan_per_load, num_rep = 2, min_fg = '', nsup = 1, fix_length = None, tile_z_dim = 3, exclude_list = [], superpix_scale = 'MIDDLE', **kwargs):
        """
        Pseudolabel dataset
        Args:
        ...
        self.nclass = len(self.pseu_label_name) # 2
        ...
        self.scan_per_load = scan_per_load  # =-1
    ...
    def get_scanids(self, mode, idx_split):
        ...
        """
        #print("((((((((((((((((((((:", idx_split)
        val_ids = copy.deepcopy(self.img_pids[self.sep[idx_split]: self.sep[idx_split + 1] + self.nsup]) # 0-6之间,0-5之间
        if mode == 'train':
    ...
    def read_dataset(self):
        ...
        for scan_id, itm in self.img_lb_fids.items():  
            ...
            for ii in range(img.shape[-1] - 1, img.shape[-1]):
                out_list.append( {"img": img[..., ii: ii + 1],  
                               "lb":lb[..., ii: ii+ 1],
                               "is_start": False,
                               "is_end": True,
    ...
    def read_classfiles(self):
        ...
        """
        with open(os.path.join(self.base_dir, f'classmap_1.json'), 'r') as fopen:
    ...
    def __getitem__(self, index):
        ...
        for ii in range(self.num_rep):  # num_rep=2
